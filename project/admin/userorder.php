    ...
    <body>
        ...
        <section>
            <article class="most-contain">
                <header>USER : <?=$_GET["user"]?></header>
                <table>
                    
                    <?php
                    $stmt=$pdo->prepare("SELECT หมายเลขออเดอร์,วันที่รับขนม,วันที่สั่งorder,SUM(การสั่งออเดอร์.จำนวนชิ้น*ขนม.ราคาขนม) AS ราคารวม FROM การสั่งออเดอร์ JOIN ขนม ON การสั่งออเดอร์.รหัสขนม=ขนม.รหัสขนม WHERE username = '".$_GET["user"]."'  GROUP BY อันดับออเดอร์");
                    $stmt->execute();
                    if($row=$stmt->fetch()):?>
                    <tr>
                    ...
                    ?>
                    <tr class="showorder">
                        <td><a href="./userorder_sub.php?user=<?=$_GET["user"]?>&order=<?=$row["หมายเลขออเดอร์"]?>"><?=$row["หมายเลขออเดอร์"]?></a></td>
                        <td><?php
                    ...
                    ?>
                    <tr><td colspan="4" class="not-found">ไม่มีข้อมูลการสั่งซื้อของลูกค้า</td></tr>
                    <?php endif;?>
                </table>
            </article>
        </section>
