...
<html>
    ...
    <script>
        ...
        function send() {
            ...
            let url = "./admin_method/showmenu_bn.php?page="+npage;
            http.open("GET", url);
            http.send();
        }
        ...
        function addmenu(){
            document.location = "./addmenu.php"
        }
        function edi_menu(pid) {
            document.location = "./editmenu.php?pid="+pid;
        }
        function close_menu(pid) {
            if(confirm("ต้องการปิดรายการขนมรหัส "+pid+" ใช่หรือไม่")==true){
            document.location = "./admin_method/close_open.php?action=close&pid="+pid;
            }
        }
    </script>
    <body onload="send()">
        ...
        <section class="order">
            <article class="list-contain">
                ...
                <?php
                $stmt=$pdo->prepare("SELECT COUNT(*) AS cou FROM ขนม WHERE สถานะ='เปิด'");
                $stmt->execute();
                $cou=$stmt->fetch();
                if($cou["cou"]>0):
